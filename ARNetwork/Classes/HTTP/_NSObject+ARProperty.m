...
static NSString *const kARAddPropertyException = @"kARAddPropertyException";

@implementation NSObject (ARProperty)
...
+ (void)_addObjectProperty:(NSString *)name {
    [self _addObjectProperty:name associationPolicy:OBJC_ASSOCIATION_RETAIN_NONATOMIC];
}


+ (void)_addObjectProperty:(NSString *)name associationPolicy:(objc_AssociationPolicy)policy {
    if (!name.length) {
        [[NSException exceptionWithName:kARAddPropertyException
                                 reason:@"property must not be empty in +_addObjectProperty:associationPolicy:"
...
+ (void)_addBasicProperty:(NSString *)name encodingType:(char *)type {
    if (!name.length) {
        [[NSException exceptionWithName:kARAddPropertyException
                                 reason:@"property must not be empty in +_addBasicProperty:encodingType:"
                               userInfo:@{@"name":name,@"type":@(type)}]
         raise];
    }
    
    if (strcmp(type, @encode(id)) == 0) {
        [self _addObjectProperty:name];
    }
    NSString *key = [NSString stringWithFormat:@"%p_%@",self,name];
    id setblock;
    id getBlock;
...
#undef blockWithCaseType
    ...
    if (!setblock || !getBlock) {
        [[NSException exceptionWithName:kARAddPropertyException
                                 reason:@"type is an unknown type in +_addBasicProperty:encodingType:"
                               userInfo:@{@"name":name,@"type":@(type)}]
         raise];
    }
